        ...
        for fa_p in fa_range:
            for sf_p in sf_range:
                ...
                for fine_ratio in fine_ratio_range:
                    ...
                    for sp in sp_range:
                        sample = pd.DataFrame([[cement, fine, coarse, fa, sf, ggbfs, sp]], columns=feature_cols)
...
if st.button("🔍 Optimize Mix"):
    ...
    if results.empty:
        ...
        comp_df = pd.DataFrame({
            ...
            "R2 Score": [r2_score(y, et_pred), r2_score(y, rf_pred)],
        ...
        for c in cement_values:
            sample = pd.DataFrame([[c, best["Fine"], best["Coarse"],
                                    best["FA"], best["SF"], best["GGBFS"], best["SP"]]],
                                  columns=feature_cols)
            sens_strength.append(et_model.predict(sample)[0])

        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=cement_values, y=sens_strength, mode='lines+markers'))
        fig2.update_layout(xaxis_title="Cement (kg)", yaxis_title="Predicted Strength")
